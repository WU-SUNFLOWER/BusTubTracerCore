...
bool ApiManager::QueryBPlusTree(ApiContext &ctx){
    ...
    std::queue<bustub::page_id_t> page_id;
    bustub::page_id_t root_page_id = index -> GetBPlusTree().GetRootPageId();
    page_id.push(root_page_id); 
    
    while(!page_id.empty()){
        rapidjson::Value resp_header(rapidjson::kObjectType);
        rapidjson::Value resp_key_value(rapidjson::kArrayType);
        auto cur_page = reinterpret_cast<bustub::BPlusTreePage*>(kBustubInstance_->buffer_pool_manager_->FetchPage(page_id.front())->GetData()); 
        page_id.pop();
        resp_header.AddMember("lsn", cur_page->GetLSN(), ctx.resp_allocator);
        ...
        if(cur_page -> IsRootPage()){
            resp_header.AddMember("parent_page_id", -1, ctx.resp_allocator);
            ...
            for(int i = 0; i < curr_page->GetSize(); i ++){
                ...
                resp_kv.AddMember("index", curr_page->KeyAt(i), ctx.resp_allocator);
                resp_kv.AddMember("page_id", curr_page->ValueAt(i), ctx.resp_allocator);
                resp_key_value.PushBack(resp_kv, ctx.resp_allocator);
                page_id.push(curr_page->ValueAt(i));
            }
            resp_root.AddMember("header", resp_header, ctx.resp_allocator);
            resp_root.AddMember("key-value", resp_key_value, ctx.resp_allocator);

        }else if(cur_page -> IsLeafPage()){
            ...
            for(int i = 0; i < curr_page->GetSize(); i ++){
                ...
                resp_kv.AddMember("index", curr_page->KeyAt(i), ctx.resp_allocator);
                resp_rid.AddMember("page_id_", curr_page->ValueAt(i).GetPageId(), ctx.resp_allocator);
                resp_rid.AddMember("slot_num_", curr_page->ValueAt(i).GetSlotNum(), ctx.resp_allocator);
                resp_kv.AddMember("rid", resp_rid, ctx.resp_allocator);
                resp_key_value.PushBack(resp_kv, ctx.resp_allocator);
            }
            resp_node.AddMember("header", resp_header, ctx.resp_allocator);
            resp_node.AddMember("key-value", resp_key_value, ctx.resp_allocator);
